...
namespace Elmah.Io.OpenTelemetry
{
    internal static class KeyValuePairExtensions
    {
        ...
        {
            statusCode = null;
            if (!(keyValue.Is("statuscode") && keyValue.Is("http.response.status_code"))) return false;
        ...
        internal static bool IsHostname(this KeyValuePair<string, object?> keyValue, out string? hostname)
        {
            return keyValue.Is("hostname", out hostname)
                || keyValue.Is("host.name", out hostname);
        }
        ...
        {
            return keyValue.Is("method", out method) || keyValue.Is("http.request.method", out method);
        }

        internal static bool IsVersion(this KeyValuePair<string, object?> keyValue, out string? version)
        {
            return keyValue.Is("version", out version) || keyValue.Is("service.version", out version);
        }

        internal static bool IsUrl(this KeyValuePair<string, object?> keyValue, out string? url)
        {
            return keyValue.Is("url", out url) || keyValue.Is("url.path", out url);
        }
        ...
        internal static bool IsRemoteAddr(this KeyValuePair<string, object?> keyValue, out string? remoteAddr)
        {
            return keyValue.Is("client.address", out remoteAddr);
        }

        internal static bool IsUserAgent(this KeyValuePair<string, object?> keyValue, out string? userAgent)
        {
            return keyValue.Is("user_agent.original", out userAgent);
        }
        ...
        {
            return new Item(property.Key, property.Value?.ToString());
        }
    }
}
